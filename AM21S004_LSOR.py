...
def Solver(Q, R):                      # P is coefficient matrix. Q is unknown vector and R is RHS which is known.
    m1 = len(Q)
    ...
    s_s = zeros(m1-3)
    r_s = zeros(m1-3)
    s_s[0] = ((omega**2)/(2*(1+(beta**2)))) - (2*(1+(beta**2)))
    for i in range(1, m1-3):
        s_s[i] = (-2*(1+(beta**2)))-((omega**2)/s_s[0])
    r_s[0] = (omega*R[0]/(2*(1+(beta**2)))) + R[1]
    for i in range(1, m1-3):
        r_s[i] = R[i+1] - ((omega*r_s[i-1])/s_s[i-1])
    x_s[m1-2] = r_s[m1-4]/s_s[m1-4]
    for i in range(m1-3, 1, -1):
        x_s[i] = (r_s[i-2]-(omega*x_s[i+1]))/(s_s[i-2])
    x_s[1] = (R[0]-(omega*x_s[2]))/(-2*(1+(beta**2)))
    return x_s


def RHS(T00, T11, T22, T10):
...
k = 1
while k < Ny-1:
    R = RHS(T[k-1, :], T[k, :], T[k+1, :], T_prev[k, :])
    T[k, :] = Solver(T[1, :], R)
    k = k+1
T = BC(T)
